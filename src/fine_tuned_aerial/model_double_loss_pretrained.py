"""
This script fine-tunes Autoencoder implementation of PyAerial (https://github.com/DiTEC-project/pyaerial) to
i) pre-train a projection encoder on TabPFN embeddings (https://github.com/PriorLabs/tabpfn),
ii) and use a double loss method to train Aerial+'s autoencoder with both a reconstruction loss and
semantic alignment (via cosine loss) to the pre-trained encoder.

This process is described in detail in our paper: Discovering Association Rules in High-Dimensional Small Tabular Data
"""
...
def pretrain_projection_encoder(autoencoder, transactions: pd.DataFrame, pretrained_embeddings, batch_size=2, epochs=25,
                                lr=5e-3, device=None, num_workers=1, noise_factor=0.5, layer_dims=[]):
    """
    Pre-train a projection encoder to align the autoencoder's encoded output with pretrained embeddings
    """
    ...
    embeddings_tensor = torch.from_numpy(pretrained_embeddings)

    dataset = TensorDataset(vectors_tensor, embeddings_tensor)
    ...
    projection_encoder = nn.Sequential(
        nn.Linear(len(autoencoder.feature_values), hidden_dim),
    ...
    optimizer = torch.optim.Adam(projection_encoder.parameters(), lr=lr, weight_decay=2e-8)

    projection_encoder.train()
    counter = 0
    best_loss = float('inf')
    for epoch in range(epochs):
        for input_batch, embedding_batch in dataloader:
            input_batch = input_batch.to(device, non_blocking=True)
            embedding_batch = embedding_batch.to(device, non_blocking=True)

            noisy_batch = (input_batch + torch.randn_like(input_batch) * noise_factor).clamp(0, 1)
            with torch.no_grad():
                reconstructed_input = autoencoder(noisy_batch,
                                                  [range(cat['start'], cat['end']) for cat in feature_value_indices])

            projected = projection_encoder(reconstructed_input)

            embedding_batch = (embedding_batch - embedding_batch.mean(dim=1, keepdim=True)) / (
                    embedding_batch.std(dim=1, keepdim=True) + 1e-8)
            loss = 1 - cos_sim(projected, embedding_batch).mean()
...
def train(transactions: pd.DataFrame, pretrained_embeddings, feature_value_indices=None,
          autoencoder: AutoEncoder = None, noise_factor=0.5, lr=5e-3, epochs=1, batch_size=2,
          loss_function=torch.nn.BCELoss(), num_workers=1, layer_dims: list = None, device=None):
    """
    ...
    projection_encoder = pretrain_projection_encoder(autoencoder, transactions, pretrained_embeddings,
                                                     batch_size=batch_size, epochs=25, layer_dims=layer_dims)

    optimizer = torch.optim.Adam(list(autoencoder.parameters()) + list(projection_encoder.parameters()), lr=lr,
                                 weight_decay=2e-8)

    total_batches = len(dataloader)
    for epoch in range(epochs):
        for batch_index, (input_batch, embedding_batch) in enumerate(dataloader):
            input_batch = input_batch.to(device, non_blocking=True)
            ...
            reconstruction_loss = loss_function(reconstructed_batch, input_batch)

            # Project reconstructed data to embedding space
            projected = projection_encoder(reconstructed_batch)

            # Embedding loss
            # normalize the embedding batch, because we are interested in vector direction (semantics) and not magnitude
            embedding_batch = (embedding_batch - embedding_batch.mean(dim=1, keepdim=True)) / (
                    embedding_batch.std(dim=1, keepdim=True) + 1e-8)
            # cos_sim to calculate vector direction similarity
            embedding_loss = 1 - cos_loss(projected, embedding_batch).mean()
